...
From HW6 in CS221, this module defines the Backtracking algorithm to solve
the CSP and some helper functions.
"""
...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    domain = [k for k in range(1, n + 1)]
    for i in range(1, n + 1):
        csp.add_variable(i, domain)
    
    for i in range(1, n):
        for j in range(i + 1, n + 1):
            csp.add_binary_factor(i, j, lambda x, y: x != y and abs(i - j) != abs(x - y))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch:
    ...
    def satisfies_constraints(self, assignment: Dict, var, val) -> bool:
        ...
        still satisfies all of the constraints.
...
############################################################
# Refactor BacktrackingSearch as BeanSearch

class BeamSearch:
    def __init__(self, beam_width: int = 1):
        self.beam_width = beam_width # i.e. K
    ...
    def backtrack(self, assignment: Dict, numAssigned: int, weight: float) -> None:
        ...
        if not self.ac3:
            ...
            for val in ordered_values:
                deltaWeight = self.get_delta_weight(assignment, var, val)
                if deltaWeight > 0:
                    deltaWeights.append((val, deltaWeight))
            
            # Keep only the top K values
            deltaWeights = sorted(deltaWeights, key=lambda x: x[1], reverse=True)[:self.beam_width]
            for val, deltaWeight in deltaWeights:
                assignment[var] = val
            ...
            for quarter in self.profile.quarters:
                csp.add_unary_factor((request, quarter), 
                                     lambda cid: cid is None \
                                        or quarter in request.quarters)
        # END_YOUR_CODE
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    # Bypass the quarter where the course is not even offered.
                    course = self.bulletin.courses[cid]
                    if not course.is_offered_in(quarter):
                        continue
                    new_variable = (cid, quarter)
                    csp.add_variable(new_variable, 
                                     [unit for unit in range(course.minUnits, course.maxUnits + 1)] + [0])
                    # Append the new variable so that we could create the sum of units constraint.
                    new_variables.append(new_variable)

                    csp.add_binary_factor((request, quarter), (cid, quarter), 
                                          lambda courseId, units: (cid != courseId and units == 0) \
                                            or (cid == courseId and units != 0))
            
            # Create aggregate var and add its factor
            create_sum_variable(csp, quarter, new_variables, self.profile.maxUnits)
            csp.add_unary_factor(("sum", quarter, "aggregated"), 
                                 lambda units: units >= self.profile.minUnits \
                                    and units <= self.profile.maxUnits)
        # END_YOUR_CODE
